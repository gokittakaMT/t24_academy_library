...
public class RentalManageController {
  ...
  public String index(Model model) {
    ...
    // 貸出一覧画面に渡すデータをmodelに追加。(RentalManageクラスでデータベースと対応しているデータを渡す。)
    // データベース内の RentalManage エンティティのすべてのレコードを取得して、それを rentalManageList
    // というリストに格納していることを意味します。
    // RentalManageはRentalManage.javaにある22行目くらいにあるpublic class RentalManageの内容
  ...
  public String add(@RequestParam(required = false) String stockId,
      @RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd") Date expectedRentalOn, Model model) {
        //@RequestParamはHTTPリクエストのパラメータを受け取るための


        List<Stock> stocks = this.stockService.findStockAvailableAll();
  ...
  public String update(@PathVariable("id") String id, @Valid @ModelAttribute RentalManageDto rentalManageDto,
      BindingResult result, RedirectAttributes ra) throws Exception {
    try {
  ...
  public String findAvailableWithRentalDate(RentalManageDto rentalManageDto, String stockId) {
    // RentalManageDtoオブジェクトとstockIdを受け取る。
  ...
  public String findAvailableWithRentalDate(RentalManageDto rentalManageDto, Long rentalId) {
    List<RentalManage> rentalAvailable = this.rentalManageService.findByStockIdAndStatusIn(rentalManageDto.getStockId(),
        rentalId);
